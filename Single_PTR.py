...
class Single_PTR:
    ...
    def cvxopt(self):
        ...
        for i in range(0,N+1) :
            if i < N :
                objective_vc.append(1 * cvx.norm(vc[i],1))
            objective.append(1 * self.cost.estimate_cost_cvx(Sx@x_cvx[i]+
                sx,Su@u_cvx[i]+su,i))
            # trust region with reference trajectory
            objective_tr.append( 1 * (cvx.quad_form(x_cvx[i] -
        ...
        l_all = self.w_c*l + self.w_vc*l_vc + self.w_tr*l_tr
    ...
    def run(self,x0,u0,xi,xf,Q=None,K=None,xi_neighbor=None,xf_margin=None):
        ...
        for iteration in range(self.maxIter) :
            ...
            if self.verbosity == True:
                print("%-12d%-18.3f%-12.3f%-12.3g%-12.3g%-12.3g%-12.3f%-1d(%2.3g)" % ( iteration+1,
                    self.w_c*self.c+self.w_vc*self.cvc+self.w_tr*self.ctr,
                    self.c,self.cvc,self.ctr,
                    expected,self.w_tr,flag_boundary,bc_error_norm))

        return self.xfwd,self.ufwd,self.xnew,self.unew,total_num_iter,flag_boundary,self.c,self.cvc,self.ctr
